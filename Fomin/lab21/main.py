...
def get_arguments():
    ...
    parser.add_argument("-p", "--prefix", type=str, help="new file prefix", default="")
...
def get_files(prefix, suffix, case_insensitive, root):
    ...
    for rootDir, dirs, files in os.walk(root):
        for file in files:
            if re.match(rf".*({suffix})(\..+)*", file, re.IGNORECASE if case_insensitive else 0):
                os.rename(rootDir + "/" + file, rootDir + "/" + prefix + file)


def main():
    get_files(*get_arguments())
