...
files_list=$(sudo find / -path "/run" -prune -o -type f -name "${prefix}*" -printf "%s %p\n"|sort -n -r)
files_size=$(echo "$files_list"|awk '{sum+=$1} END {print sum}')
...
do
        biggest_file=$(echo "$files_list"|head -n 1|awk '{print $2}')
        ...
        if [[ -r "$biggest_file" ]]; then
                ...
                files_list=$(echo "$files_list"|tail -n +2)
        else
                files_list=$(echo "$files_list"|tail -n +2)
        fi
